...
/**
 * Async, fallible result — `Future[Either[DomainError, A]]` with `map`/`flatMap` so services can chain
 * validation + persistence in one for-comprehension (first `Left` short-circuits). Mirrors the todo
 * pattern, re-created under `drp` (DRP must not import `app/todo`).
 */
