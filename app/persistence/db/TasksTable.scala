...
/**
 * `tasks` tablosu + duz [[TaskRow]].
 *
 * DOMAIN != DB: domain'in `urgency` ADT'si burada YOK -> `priorityValue` (Int) +
 * `dueDate` (Date). Zaman tipleri JDBC-yerel (`Timestamp`/`SqlDate`). Zengin
 * domain'e cevrim `persistence.db.mappers.TaskMappers`'ta.
 */
